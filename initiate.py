...
stylesheet = """
    MainWindow {
        background-image: url("C:/Users/prana/AppData/Local/Programs/Python/Python39/Lib/site-packages/win64py/temp.png");
    }
"""
def killprocess():
    """this is only for Snipping tools don't use it!!"""
    def check_pid(pid):        
        try:
...
def Snip_only(wait_time):
    c = pyautogui.screenshot()
    c.save("temp.png")
    app2hello = QApplication(sys.argv)
    app2hello.setStyleSheet(stylesheet)     # <---
    ...
    mainwindow_id = subprocess.run(["tasklist", "/v", "/fo", "csv", "|", "findstr", "/i", '"hellofromwin64py"'], check_output=True).read()
    
    pyautogui.sleep(wait_time-1)
    killprocess()
    sys.exit(app.exec_(), app2hello.exec_())
